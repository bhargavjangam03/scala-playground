...
object EmailUtils {
  ...
  val senderEmail = Option(config.getString("settings.sender-email")).getOrElse("bhargavjangam03@gmail.com")
  val senderName = config.getString("settings.sender-name")
  val senderPassword = Option(config.getString("settings.sender-password")).getOrElse("aoah aovq wmgc iwcj")
